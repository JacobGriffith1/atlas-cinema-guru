...
export async function POST(_: Request, ctx: { params: { id: string } }) {
  ...
  try {
    await query(`INSERT INTO favorites (user_id, title_id) VALUES ($1, $2)`, [userId, titleId]);
    await logActivity(userId, "FAVORITE_ADD", titleId);
...
export async function DELETE(_: Request, ctx: { params: { id: string } }) {
  ...
  const titleId = ctx.params.id;
  const res = await query(`DELETE FROM favorites WHERE user_id = $1 AND title_id = $2 RETURNING 1`, [
    userId,
    titleId
  ]);
  if (res.length === 0) return notFound("Not in favorites");

  await logActivity(userId, "FAVORITE_REMOVE", titleId);
