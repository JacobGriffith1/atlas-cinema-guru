...
export default function WatchLaterPage() {
  ...
  async function load(nextPage: number) {
    ...
    setError("");
    try {
      const res = await fetch(`/api/watch-later?page=${nextPage}`, { cache: "no-store" });
  ...
  async function toggleWatchLater(id: string, next: boolean) {
    ...
    // If you remove from Watch Later page, remove it from the list immediately.
    if (!next) {
      setData((prev) => {
        if (!prev) return prev;
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-semibold">Watch Later</h1>
        <p className="mt-1 text-sm text-neutral-300">Movies you’ve saved for later.</p>
      </div>
      ...
      {data && data.items.length === 0 ? (
        <div className="text-sm text-neutral-400">No watch-later movies yet.</div>
      ) : null}
